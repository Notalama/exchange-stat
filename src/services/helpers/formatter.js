...
module.exports = {
  formatRates: async (unformattedList) => {
    try {
      ...
      const checkBonus = function (changer) {
        return bonuses.find(bon => bon.changer === changer)
      }
      const calcBonus = function (rate, bonus) {
          ...
          for (let j = 0; j < byCurr[id].length; j++) {
            if (byCurr[id][j]) {
              ...
              if (rowArray[1] === el[1]) {
                ...
                if (isProfitable) {
                  const bonus = checkBonus(rowArray[2])
                  if (bonus) {
                    rowArray = calcBonus(rowArray, bonus)
                  }
